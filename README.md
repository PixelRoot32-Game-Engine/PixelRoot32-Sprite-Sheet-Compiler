...
`main.py` is a Python tool that provides both a **command-line interface** and a **GUI** to convert a **PNG sprite sheet** into a C header file (`.h`) compatible with the **PixelRoot32** engine.
...
For most users (artists, technical designers), the GUI is the easiest way to use the compiler. It is a thin wrapper around the CLI logic and internally uses the same parameters.

#### 3.1.1 Launching the GUI

- From a terminal in this folder (run without arguments):
...
- **Compile**  
  - Runs the compilation logic with the parameters defined in the GUI.  
  - The button is disabled while compilation is in progress and re‑enabled when it finishes.

- **Log**  
  - Shows:
    - The exact command equivalent used for the compilation.
    - Progress and output produced by the compiler.
...
```powershell
python .\main.py "assets\player sprites.png" --grid 16x32 ...
```
...
```powershell
python .\main.py ... --out sprites_player.h
```
...
From the project root (where [main.py](main.py) lives), run:

```powershell
pyinstaller --noconsole --onefile --add-data "src\assets;src\assets" main.py
```

Notes:

- `--onefile` produces a single `main.exe` (or your chosen name).
- `--noconsole` hides the console window when launching the GUI.
- `--add-data "src\assets;src\assets"` embeds the required assets (logo, etc.) inside the binary.
...
The resulting binary will be placed in the `dist/` folder (e.g. `dist/main`).

> The tool has been written to work both as a plain Python script and as a PyInstaller-frozen executable. It automatically detects if it should run in CLI mode (if arguments are passed) or GUI mode.
